...
export default function EONotificationsPage() {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <Bell className="w-6 h-6 text-emerald-600" /> Notifikasi EO
          </h1>
          <p className="text-sm text-slate-500 mt-1">Update status approval event dan diskusi admin.</p>
        </div>
              ...
              <div key={n.id} className={`px-5 py-4 border-b border-slate-100 ${n.isRead ? 'bg-white' : 'bg-emerald-50/40'}`}>
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    {n.data?.eventId && (
                      <Link
                        href={`/eo/events/${encodeURIComponent(n.data.eventId)}/manage?tab=diskusi`}
