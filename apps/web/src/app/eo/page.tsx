...
export default function EODashboardPage() {
  ...
  const [stats, setStats] = useState<Stats>({ totalEvents: 0, totalSold: 0, totalRevenue: 0 });
  ...
  useEffect(() => {
    if (user?.role === 'EO_ADMIN' || user?.role === 'EO_STAFF') {
      api.get('/api/eo/events').then((res) => {
        const data = res.data?.data || res.data;
        setStats((s) => ({ ...s, totalEvents: Array.isArray(data) ? data.length : 0 }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Link href="/eo/events" className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 hover:border-[#065F46] transition-colors">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-lg bg-[#065F46]/10 flex items-center justify-center">
              <Calendar className="w-6 h-6 text-[#065F46]" />
            </div>
            <div>
        ...
        <Link href="/eo/staff" className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 hover:border-[#065F46] transition-colors">
          <div className="flex items-center gap-4">
        ...
        <Link href="/eo/profile" className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 hover:border-[#065F46] transition-colors">
          <div className="flex items-center gap-4">
