...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (error === 'oauth_failed') {
      toast.showToast('error', 'Login Google gagal, coba lagi');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (redirect === '/dashboard' || !redirect) {
        ...
        } else if (currentUser?.role === 'EO_ADMIN') {
          redirectPath = '/eo';
        } else {
          redirectPath = '/dashboard';
        }
      } else if (redirect === '/admin' && currentUser?.role === 'SUPER_ADMIN') {
        redirectPath = '/admin/users';
      ...
      if (err.message === 'EMAIL_VERIFICATION_REQUIRED') {
        ...
        if (apiError.code === 'EMAIL_NOT_VERIFIED') {
          ...
          setServerError('Akun terkunci. Coba lagi dalam 30 menit.');
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        >
          ...
          {googleLoading ? 'Redirecting to Google...' : 'Lanjutkan dengan Google'}
        </Button>
